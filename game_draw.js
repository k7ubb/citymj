...
const calcKansRect = () => {
	...
	for (let i = 0; i < 4; i++) {
		...
		for (let j = 0; j < 4; j++) {
			rects[i].push([start_x + KAN_W * j, 900 - KAN_W * 4 / 3, KAN_W - lineWidth, KAN_W * 4 / 3]);
		}
	}
	return rects;
};
...
const drawDraggingArrow = (rect) => {
	...
	const y = rect[1];
	$.ctx.bbFill(
		$.path({points:[ [x, y - 30], [x - 20, y - 60], [x + 20, y - 60]]}),
		"#cc0"
	);
};

const drawCheckbox = (x, y, isChecked) => {
	$.ctx.bbStroke($.path({rect: [x, y, 40, 40]}), "#000", lineWidth * 2);
	if (isChecked) {
...
const drawKanButton = (drawObject, isHover) => {
	...
	$.ctx.bbFill($.path({ rect: [x, y + 15, w, h - 30], radius: 10}), isHover? "#999" : "#ccc");
...
const drawCityTable = (hand, character) => {
  ...
  const cities = CITIES.filter(city => city.name.includes(character));
	$.ctx.bbFill($.path({rect: [start_x - 15, RECT_Y - 15, 1600 - start_x * 2 + 30, TEXT_SIZE * 1.5 * TEXT_LINES + 10]}), COLOR_STRONG);
	$.ctx.bbFill($.path({rect: [start_x - 10, RECT_Y - 10, 1600 - start_x * 2 + 20, TEXT_SIZE * 1.5 * TEXT_LINES]}), "#fff");
