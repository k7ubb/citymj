...
CanvasRenderingContext2D.prototype.bbText = function(text, x, y, {color='#000000', size=14, font='sans-serif', style='', align='left', baseline='top', rotate} = {}) {
	...
	const y_ = y - size * .1 + (baseline === "middle" && size / 2) + (baseline === "top" && size);
	if (rotate) {
		this.rotate(rotate);
		this.fillText(
			text,
			Math.cos(rotate) * x + Math.sin(rotate) * y_,
			-Math.sin(rotate) * x + Math.cos(rotate) * y_
		);
		this.rotate(-rotate);
	}
	else {
		this.fillText(text, x, y_);
	}
	this.font = font_;
	this.fillStyle = fillStyle_;
